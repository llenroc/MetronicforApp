...
<html>
    ...
    <body ng-app="BrowserSync"> %svg%
        <disconnect-elem></disconnect-elem>
        <notify-elem></notify-elem>
        <main ng-controller="MainController as app"> %header%
                        ...
                        <ul bs-list="stacked">
                            <li ng-class="{'active': item.active}" ng-repeat="(key, item) in app.ui.menu | orderObjectBy: 'order'">
                                <button bs-button ng-class="{'active': item.active}" ng-click="app.setActiveSection(item)">
                                    <icon icon="{{item.icon}}"></icon> {{item.title | ucfirst}} </button>
                            </li>
                            <li bs-action="menu-close" bs-visible="palm">
                                <button bs-button="nav" ng-click="app.toggleMenu()">
                                    <icon icon="circle-delete"></icon> CLOSE</button>
                            </li>
                        </ul>
                    </nav>
                    <div bs-visible="not-palm"> %footer% </div>
                </section>
                <section bs-content> %pageMarkup% %templates%
                    <div ng-view></div>
                    <div bs-visible="palm"> %footer% </div>
                </section>
            </section>
        </main>
        <!--
